...
r_III = 30.0 # radius of region III. Other regions are derived from this
...
# @assert maximum((central_fdm(2, 1))(α -> u(1.0, α), 1.2) - ∇u(1.0, 1.2)) < 1e-6 # FIXME !!
...
p = plot(; yscale=:log10, xscale=:log10, legend=:bottomleft, xlabel="r",
            ylabel="|f|")
for (label, calculator) in zip(("JuLIP EAM", ),
                                (eam, ))
    @show label
    set_calculator!(clust, calculator)
    scatter!(r, norm.(forces(clust)), label=label)
end
plot!(r, 8*r.^(-1.5), label="r^-1.5", lw=2)
p
savefig("eam.png")