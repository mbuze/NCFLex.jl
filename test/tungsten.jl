...
eam = JuLIP.Potentials.EAM(@__DIR__() * "/W.eam.fs")
...
@show C11, C12, C44
@test all(isapprox.((C11, C12, C44), (3.2613049783946484, 1.2758603876297692, 1.0024933756887762), atol=1e-5))
...
@test γ / (ase_units.J / ase_units.m^2) ≈ 2.926651712200981
...
mask = [all([jj ∉ surface_atoms for jj in j]) for (i,j,R) in sites(clust, cutoff(eam)+1)]
